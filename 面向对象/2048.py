...
class GameInit(object):
    ...
    def drawBlock(screen,row,column,color,blocknum):
        ...
        w = column*Block_WH+(column+1)*BLock_Space    #x坐标
        h = row*Block_WH+(row+1)*BLock_Space+110      #y坐标
        pygame.draw.rect(screen,color,(w,h,110,110))  #绘制矩形
        if blocknum != 0:
            fw,fh = font.size(str(int(blocknum)))
            screen.blit(font.render(str(int(blocknum)),True,(0,0,0)),(w+(110-fw)/2,h+(110-fh)/2))   #将一个图像绘制到另一个
...
def main():
    ...
    matrix = GameInit.initData(Size)   #返回初始化任意位置为2或者4的矩阵
    print(matrix)
    currentscore = 0
